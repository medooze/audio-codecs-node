...
class AudioEncoder extends Emitter
{
	...
	createIncomingStreamTrack()
	{
		...
		this.encoder.AddListener(bridge.toMediaFrameListener()[SharedPointer.Pointer]);
		//Add listener
		track.once("stopped",(track)=>{
			//Remove source from listener
			this.encoder.RemoveListener(bridge.toMediaFrameListener()[SharedPointer.Pointer]);
			//Stop bridge
			bridge.Stop();
			//Remove from map
